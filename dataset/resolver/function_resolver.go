...
func (r *InMemFunctionRegistry) seedDefaultFunctions() {
	...
	add("COUNT_ALL", domain.CategoryAggregate, 0, 0, true, "COUNT(*)", "COUNT(*)", "{\"$sum\": 1}", "Counts all rows")
	...
	add("DATEDIFF", domain.CategoryDateTime, 2, 2, false, "({{0}}::date - {{1}}::date)", "DATEDIFF({{0}}, {{1}})", "{\"$dateDiff\": {\"startDate\": \"{{1}}\", \"endDate\": \"{{0}}\", \"unit\": \"day\"}}", "Difference in days")
